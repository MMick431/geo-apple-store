...
module.exports = async function handler(req, res) {
  ...
  const waMsg = encodeURIComponent(`Bonjour ${sName}, j'ai bien reçu votre commande pour ${sProduct}. Je reviens vers vous rapidement ! - GEO STORE`);
...
<table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0a;padding:40px 20px;">
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="background:#141414;border-radius:20px;overflow:hidden;border:1px solid #222;">
      <tr><td style="background:linear-gradient(135deg,#1428A0,#0a1870);padding:36px 40px;text-align:center;">
        <p style="margin:0;color:rgba(255,255,255,0.5);font-size:12px;letter-spacing:3px;text-transform:uppercase;">Nouvelle Commande Samsung</p>
        <h1 style="margin:8px 0 0;color:#fff;font-size:26px;font-weight:900;letter-spacing:2px;">GEO STORE</h1>
      </td></tr>
      <tr><td style="padding:0 40px;">
        <div style="background:#1e1e1e;border:1px solid #1428A033;border-radius:10px;padding:16px 20px;margin:24px 0;">
          <span style="color:#1428A0;font-weight:700;font-size:13px;text-transform:uppercase;">Commande Samsung — Traitement requis</span>
        </div>
      </td></tr>
      <tr><td style="padding:0 40px;">
        <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a1a;border-radius:12px;border:1px solid #333;">
          <tr><td style="padding:20px 24px;">
            ...
            <p style="margin:6px 0 0;color:#1428A0;font-size:22px;font-weight:900;">${sPrice} FCFA</p>
          </td></tr>
        </table>
      </td></tr>
  ...
  try {
    ...
    const data = await resend.emails.send({
      from: "GEO Store <onboarding@resend.dev>",
      to: [process.env.STORE_EMAIL || "michaelhologan45@gmail.com"],
      subject: `Commande Samsung — ${sProduct} | GEO STORE`,
      html,
      reply_to: sEmail,
    });
    res.json({ success: true, emailId: data.id });
  } catch (error) {
    console.error("Erreur Resend Samsung:", error);
